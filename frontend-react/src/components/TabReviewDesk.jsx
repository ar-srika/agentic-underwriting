...
import MemoryBankCard from './MemoryBankCard';

export default function TabReviewDesk({ decision, onOverrideDecision, onHydrateSession }) {
  const [comments, setComments] = useState('');
  const [overrideStatus, setOverrideStatus] = useState(null);

  const handleAction = async (action) => {
    setOverrideStatus(action);
    if (onOverrideDecision && decision) {
  ...
  return (
    <div>
      <div className="enterprise-card">
        ...
        {!decision ? (
          <div style={{ padding: '24px', textAlign: 'center', color: '#64748b' }}>
            No active submission in the live underwriter review queue. Select or re-hydrate a session from the Memory Bank below.
          </div>
        ) : (
          <>
      ...
      {/* Memory Bank 90-Day Cold Storage Snapshots */}
      <MemoryBankCard onHydrateSession={onHydrateSession} />
    </div>
  );
}
